...
if [ "$color_prompt" = yes ]; then
    ...
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u:\[\033[01;34m\]\W\[\033[38;5;172m\] > \[\033[00m\]'
else
...
alias mkdir="mkdir -p"	# Make all parent dirs as needed

# Add an "alert" alias for long running commands.  Use like so:
#   sleep 10; alert
alias alert='notify-send --urgency=low -i "$([ $? = 0 ] && echo terminal || echo error)" "$(history|tail -n1|sed -e '\''s/^\s*[0-9]\+\s*//;s/[;&|]\s*alert$//'\'')"'

# Some local config that I don't want to put on GitHub
...
# Launch tmux by default if (1) tmux exists, (2) interactive shell and (3) not within tmux
